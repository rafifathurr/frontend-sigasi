...
@section('content')
    <div class="container-fluid px-0">
        ...
        <div class="card shadow-sm border-0 w-100">
            <form action="{{ route('penduduk.update', $data->penduduk->IDPenduduk) }}" method="POST" id="form-submit">
                @method('PUT')
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="form-group mb-3 col-lg-6">
                            ...
                            <input type="number" value="{{ $data->penduduk->KTP }}" class="form-control" name="ktp"
                                id="ktp" required>
                        </div>
                        <div class="form-group mb-3 col-lg-6">
                            <label for="nama" class="form-label">Nama Lengkap<span
                                    class="ms-1 text-danger">*</span></label>
                            <input type="text" value="{{ $data->penduduk->Nama }}" class="form-control" name="nama"
                                id="nama" required>
                        </div>
                        <div class="form-group mb-3 col-lg-6">
                            ...
                            <select class="w-100 select2" name="jenis_kelamin" required>
                                ...
                                <option value="0" {{ $data->penduduk->JenisKelamin == 0 ? 'selected' : '' }}>Laki
                                    - Laki</option>
                                <option value="1" {{ $data->penduduk->JenisKelamin == 1 ? 'selected' : '' }}>
                                    Perempuan</option>
                            </select>
                        </div>
                        <div class="form-group mb-3 col-lg-6">
                                ...
                                @else
                                    ...
                                    @foreach ($data->kelompok as $item)
                                        <option value="{{ $item->IDKelompok }}"
                                            {{ $item->IDKelompok == $data->penduduk->kelompok->IDKelompok ? 'selected' : '' }}>
                                            {{ $item->NamaKelompok }}
                                        </option>
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group mb-3 col-lg-6">
                            <label for="tanggal_lahir" class="form-label">Tanggal Lahir<span
                                    class="ms-1 text-danger">*</span></label>
                            <input type="date"
                                value="{{ \Carbon\Carbon::parse($data->penduduk->TanggalLahir)->format('Y-m-d') }}"
                        ...
                        <div class="form-group mb-3 col-lg-6">
                            ...
                            <input type="text" value="{{ $data->penduduk->Desa }}" class="form-control" name="desa"
                                id="desa" required>
                        </div>
                    </div>
                    <div class="form-group mb-3 col-lg-12">
                        <label for="alamat" class="form-label">Alamat Lengkap<span
                                class="ms-1 text-danger">*</span></label>
                        <textarea class="form-control" name="alamat" id="alamat" rows="3" required>{{ $data->penduduk->Alamat }}</textarea>
                    </div>
                </div>
