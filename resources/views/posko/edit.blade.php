...
@section('content')
    <div class="container-fluid px-0">
        <h4 class="fw-bold py-3"><span class="text-muted fw-light">Posko /</span> Edit Posko</h4>
        <div class="card shadow-sm border-0 w-100">
            <form action="{{ route('posko.update', $data->posko->IDPosko) }}" method="POST" id="form-submit">
                @method('PUT')
                @csrf
                <div class="card-body">
                    <div class="row">
                                ...
                                @else
                                    ...
                                    @foreach ($data->users as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $data->posko->Ketua ? 'selected' : '' }}>{{ $item->name }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
                        ...
                        <div class="form-group mb-3 col-12">
                            ...
                            <textarea class="form-control" name="solution" rows="3" required>{{ $data->posko->SolusiMasalah }}</textarea>
                        </div>
                    </div>
                </div>
